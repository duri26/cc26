...
void DictProducer::getAllFiles(vector<string> & files
		,const string & path)
{
	DIR * dir = opendir(path.c_str());
	if(!dir)
	{
		LogError("No such dirctonary");
	}
...
void DictProducer::getStopWordsDict(const string & path)
{
	...
	for(const string & file : files)
	{
		...
		{
			LogWarn("Open stopwords dictionary file \"%s\" failed",file.c_str());
			input.close();
			return;
		}
...
void DictProducer::buildIndex()
{
	...
	for(const auto & record : _dict)
	{
				...
				cit != record.first.cend();)
		{
			...
			if(it == _index.end())
			{
				_index.insert(std::make_pair(std::move(s)
							,set<size_t>{idx}));
			}
			else
			{
